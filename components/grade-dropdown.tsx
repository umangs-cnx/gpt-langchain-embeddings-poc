...
export default function GradeDropdown(props: any) {
    const [ selectedItemName, setSelectedItemName ] = useState('Grade');
    ...
    const menuList = useMemo(() => {
      ...
      const menuList = grades.map((grade) =>
        <Menu.Item key={grade}>
          {({ active }) => (
            <a
              onClick={(e) => props.dropdownItem(grade)}
              href="#"
    ...
    useEffect(() => {
        if (props.selectedItem) setSelectedItemName(`Grade ${props.selectedItem}`);
        else setSelectedItemName('Grade');
    }, [props.selectedItem]);
    return (
